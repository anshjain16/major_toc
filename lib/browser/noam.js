	...
	(function () {
		...
		(function () {
			...
			function _defaultHash(obj) {
				var h, i;
				if (obj === undefined) {
					// undefined might be a "value" of some property
				...
				for (i = 0; i < props.length; i++) {
					...
					h =
						((((h * 39) ^ _defaultHash(props[i])) * 43) ^
							_defaultHash(obj[props[i]])) &
						HASH_MASK;
				}
				return h;
			}
			...
			structure.HashTable = function (cfg) {
				...
				if (cfg) {
					...
					if (cfg.hash) {
						// otherwise we use the prototype one
						this.hash = cfg.hash;
					}
					if (cfg.equals) {
						// otherwise we use the prototype one
			...
			structure.HashTable.prototype.containsKey = function (key) {
				if (key === undefined) {
					throw new Error(
						"called HashTable.containsKey with key === undefined"
					);
				}
			...
			var Iterator = {
				hasNext: function () {
					...
					if (
						...
						this.cur_chain_iterator.hasNext()
					) {
						return true;
					}
					...
					while (
						this.idx < this.H.capacity &&
						(this.H.slots[this.idx] === undefined || // no chain OR
							this.H.slots[this.idx].isEmpty())
					) {
						// empty chain
						++this.idx;
					}
				...
				each: function (iterator_func, context) {
					if (iterator_func === undefined) {
						throw new Error(
							"called iterator.each with iterator_func === undefined"
						);
					}
			...
			_KeyIterator.prototype.next = function () {
				return this._next(
					"KeyIterator.next called on empty iterator",
					function (it) {
			...
			_ValueIterator.prototype.next = function () {
				return this._next(
					"ValueIterator.next called on empty iterator",
					function (it) {
			...
			_KeyValueIterator.prototype.next = function () {
				return this._next(
					"KeyValueIterator.next called on empty iterator",
					function (it) {
		...
		noam.fsm._addStateOrSymbol = function (
			arr,
			obj,
			undefErrorMsg,
			existsErrorMsg
		...
		noam.fsm._addTransition = function (
			...
			if (
				!noam.util.contains(fsm.states, fromState) ||
				!noam.util.containsAll(fsm.states, toStates)
			) {
				throw new Error(
					"One of the specified objects is not a state of the FSM"
				);
			}
			...
			for (i = 0; i < fsm.transitions.length; i++) {
				if (
					noam.util.areEquivalent(fromState, fsm.transitions[i].fromState) &&
					noam.util.areEquivalent(transitionSymbol, fsm.transitions[i].symbol)
				) {
					fsm.transitions[i].toStates = noam.util.setUnion(
		...
		noam.fsm.addTransition = function (
			fsm,
			fromState,
			toStates,
			transitionSymbol
		...
		noam.fsm.validate = function (fsm) {
			...
			for (i = 0; i < fsm.acceptingStates.length; i++) {
				if (
					noam.util.contains(fsm.acceptingStates, fsm.acceptingStates[i], i + 1)
				) {
			...
			for (i = 0; i < fsm.transitions.length; i++) {
				...
				if (
					...
					throw new Error(
						"Transitions must have fromState, toState and symbol"
					);
				}
				...
				if (
					!noam.util.contains(fsm.alphabet, transition.symbol) &&
				...
				for (var i = 0; i < fsm.transitions.length; i++) {
					...
					if (
						...
						for (var j = 0; j < transition.toStates.length; j++) {
							if (
								noam.util.contains(targetStates, transition.toStates[j]) ||
		...
		noam.fsm.makeSimpleTransition = function (fsm, states, symbol) {
			...
			for (var i = 0; i < fsm.transitions.length; i++) {
				...
				if (
					noam.util.areEquivalent(fsm.transitions[i].symbol, symbol) &&
		...
		noam.fsm.printTable = function (fsm) {
			...
			for (i = 0; i < fsm.states.length; i++) {
				...
				tableRows[i][colHeads.length - 1] = noam.util.contains(
					fsm.acceptingStates,
					fsm.states[i]
				)
					? "1"
		...
		noam.fsm.parseFsmFromString = function (fsm_string) {
			...
			for (var k in parseCounts) {
				if (parseCounts[k] !== 1) {
					throw new Error(
						"Specification missing #" + parseCounts[k] + " section."
					);
				}
			}
				...
				for (var j = 0; j < transition[0].length; j++) {
					for (var k = 0; k < transition[1].length; k++) {
						if (transition[1][k] === noam.fsm.epsilonSymbol) {
							noam.fsm.addEpsilonTransition(
								fsm,
								transition[0][j],
								transition[2]
							);
						} else {
							noam.fsm.addTransition(
								fsm,
								transition[0][j],
								transition[2],
								transition[1][k]
		...
		noam.fsm.printHtmlTable = function (fsm) {
			...
			for (i = 0; i < fsm.states.length; i++) {
				...
				tableRows[i][headers.length - 1] = noam.util.contains(
					fsm.acceptingStates,
					fsm.states[i]
				)
					? {text: ["1"]}
				...
				for (j = 0; j < fsm.transitions[i].toStates.length; j++) {
					...
					for (k = 0; k < newTransitions.length; k++) {
						if (
							...
							noam.util.areEquivalent(newTransitions[k].toStates, [
		...
		noam.fsm.getReachableStates = function (
			fsm,
			state,
			shouldIncludeInitialState
		) {
				...
				for (i = 0; i < fsm.transitions.length; i++) {
					...
					if (noam.util.areEquivalent(currentState, transition.fromState)) {
						for (j = 0; j < transition.toStates.length; j++) {
							if (
								!noam.util.contains(reachableStates, transition.toStates[j])
							) {
		...
		noam.fsm.removeUnreachableStates = function (fsm) {
			var reachableStates = noam.fsm.getReachableStates(
				fsm,
				fsm.initialState,
				true
			);
		...
		noam.fsm.areEquivalentStates = function (fsmA, stateA, fsmB, stateB) {
			if (
				noam.fsm.determineType(fsmA) !== noam.fsm.dfaType ||
			...
			if (
				!noam.util.contains(fsmA.states, stateA) ||
			...
			while (unprocessedPairs.length !== 0) {
				...
				for (j = 0; j < fsmA.alphabet.length; j++) {
					...
					if (
						!noam.util.contains(processedPairs, pair) &&
		...
		noam.fsm.areEquivalentFSMs = function (fsmA, fsmB) {
			return noam.fsm.areEquivalentStates(
				fsmA,
				fsmA.initialState,
				fsmB,
				fsmB.initialState
		...
		noam.fsm.removeEquivalentStates = function (fsm) {
			...
			for (i = 0; i < fsm.states.length; i++) {
				for (j = i + 1; j < fsm.states.length; j++) {
					if (
						noam.fsm.areEquivalentStates(fsm, fsm.states[i], fsm, fsm.states[j])
					) {
			...
			newFsm.initialState = noam.util.clone(
				getEquivalentState(fsm.initialState)
			);
		...
		noam.fsm.minimize = function (fsm) {
			...
			var fsmWithoutUnreachableStates =
				noam.fsm.removeUnreachableStates(newFsm);
			var minimalFsm = noam.fsm.removeEquivalentStates(
				fsmWithoutUnreachableStates
			);
			return minimalFsm;
		};
		...
		// generate random fsm
		noam.fsm.createRandomFsm = function (
			fsmType,
			numStates,
			numAlphabet,
			maxNumToStates
		...
		noam.fsm.convertNfaToDfa = function (fsm) {
			...
			while (newStates.length !== 0) {
				...
				for (i = 0; i < newFsm.alphabet.length; i++) {
					var ts = noam.fsm
						.makeTransition(fsm, state, newFsm.alphabet[i])
						.sort();
					...
					if (
						!noam.util.containsSet(newFsm.states, ts) &&
						!noam.util.containsSet(newStates, ts) &&
						ts.length > 1
					) {
						newStates.push(ts);
					}
			...
			for (i = 0; i < newFsm.states.length; i++) {
				for (j = 0; j < newFsm.alphabet.length; j++) {
					...
					for (k = 0; k < newFsm.transitions.length; k++) {
						...
						if (
							noam.util.areEquivalent(transition.symbol, newFsm.alphabet[j]) &&
		...
		noam.fsm.convertEnfaToNfa = function (fsm) {
			...
			var initialEpsilon = noam.fsm.computeEpsilonClosure(fsm, [
				fsm.initialState,
			]);

			if (
				noam.util.containsAny(newFsm.acceptingStates, initialEpsilon) &&
			...
			for (i = 0; i < newFsm.transitions.length; i++) {
				...
				if (transition.toStates.length > 1) {
					...
					for (j = 0; j < multiStateTransitions.length; j++) {
						if (
							noam.util.areEqualSets(
								transition.toStates,
								multiStateTransitions[j]
							)
						) {
		...
		noam.fsm.isLanguageInfinite = function (fsm) {
			...
			for (i = 0; i < newFsm.states.length; i++) {
				...
				reachable = noam.fsm.getReachableStates(
					newFsm,
					newFsm.states[i],
					false
				);
		...
		noam.fsm.randomStringInLanguage = function (fsm) {
			...
			while (true) {
				if (
					noam.util.areEquivalent(currentState, newFsm.initialState) === true
				) {
					if (Math.round(Math.random())) {
						break;
					}
				...
				for (i = 0; i < newFsm.transitions.length; i++) {
					if (
						noam.util.areEquivalent(
							newFsm.transitions[i].toStates[0],
							currentState
						)
					) {
		...
		noam.fsm.union = function (fsmA, fsmB) {
			...
			var newFsm = {
				...
				states: [],
				initialState: [
					noam.util.clone(fsmA.initialState),
					noam.util.clone(fsmB.initialState),
				],
				acceptingStates: [],
				transitions: [],
			};
			...
			for (i = 0; i < fsmA.states.length; i++) {
				for (j = 0; j < fsmB.states.length; j++) {
					var newState = [
						noam.util.clone(fsmA.states[i]),
						noam.util.clone(fsmB.states[j]),
					];
					newFsm.states.push(newState);

					if (
						noam.util.contains(fsmA.acceptingStates, fsmA.states[i]) ||
		...
		noam.fsm.intersection = function (fsmA, fsmB) {
			var new_alphabet = noam.util.clone(
				noam.util.setIntersection(fsmA.alphabet, fsmB.alphabet)
			);
		...
		noam.fsm.symbolsForTransitions = function (fsm, stateA, stateB) {
			...
			for (var i = 0; i < fsm.transitions.length; i++) {
				...
				if (
					noam.util.areEquivalent(transition.fromState, stateA) &&
		...
		noam.fsm.toRegex = function (fsm) {
			...
			for (i = 0; i < n; i++) {
				for (j = 0; j < n; j++) {
					var symbols = noam.fsm.symbolsForTransitions(
						fsm,
						fsm.states[i],
						fsm.states[j]
					);
				...
				for (i = 0; i < n; i++) {
					for (j = 0; j < n; j++) {
						...
						if (r[k - 1][k - 1][k - 1].tag === noam.re.tree.tags.EPS) {
							...
							seq = noam.re.tree.makeSeq([
								r[k - 1][i][k - 1],
								noam.re.tree.makeKStar(r[k - 1][k - 1][k - 1]),
								r[k - 1][k - 1][j],
							]);
						}
		...
		noam.grammar.validate = function (grammar) {
			if (
				!(
					...
					typeof grammar.initialNonterminal !== "undefined" &&
					grammar.initialNonterminal !== null &&
					Array.isArray(grammar.productions)
				)
			) {
				throw new Error(
					"Grammar must be defined and have nonterminals, terminals, initialNonterminal and productions array properties!"
				);
			}
			...
			for (i = 0; i < grammar.terminals.length; i++) {
				if (
					noam.util.contains(grammar.terminals, grammar.terminals[i], i + 1)
				) {
			...
			if (
				!noam.util.contains(grammar.nonterminals, grammar.initialNonterminal)
			) {
			...
			for (i = 0; i < grammar.productions.length; i++) {
				...
				if (production.left.length === 0) {
					throw new Error(
						"Left side of production must have at least one terminal or nonterminal"
					);
				}

				for (j = 0; j < production.left.length; j++) {
					if (
						!noam.util.contains(grammar.nonterminals, production.left[j]) &&
						!noam.util.contains(grammar.terminals, production.left[j])
					) {
						throw new Error(
							"Left side of production must be in nonterminals or terminals"
						);
					}
				}
				...
				if (
					...
					production.right[0] === noam.grammar.epsilonSymbol
				) {
				} else {
					if (production.right.length === 0) {
						throw new Error(
							"Right side of production must have at least one terminal or nonterminal or epsilon symbol"
						);
					}

					for (j = 0; j < production.right.length; j++) {
						if (
							!noam.util.contains(grammar.nonterminals, production.right[j]) &&
							!noam.util.contains(grammar.terminals, production.right[j])
						) {
							throw new Error(
								"Right side of production must be in nonterminals or terminals"
							);
						}
					}
				}
			...
			for (i = 0; i < grammar.productions.length; i++) {
				...
				if (type === noam.grammar.regType) {
					if (
						...
						!noam.util.contains(grammar.nonterminals, production.left[0])
					) {
				...
				if (type === noam.grammar.csgType) {
					...
					for (j = 0; j < sufix.length; j++) {
						if (
							!noam.util.areEquivalent(
								sufix[sufix.length - j - 1],
								production.right[production.right.length - j - 1]
							)
						) {
							return noam.grammar.unrestrictedType;
						}
		...
		noam.grammar.printAscii = function (grammar) {
			...
			str.push(
				"Initial nonterminal: " + "<" + grammar.initialNonterminal + ">"
			);
			...
			for (i = 0; i < grammar.productions.length; i++) {
				...
				if (foundSlim === -1) {
					slimProds[slimProds.length] = [
						grammar.productions[i].left,
						[grammar.productions[i].right],
					];
				} else {
			...
			for (i = 0; i < slimProds.length; i++) {
				...
				for (j = 0; j < slimProds[i][1].length; j++) {
					for (k = 0; k < slimProds[i][1][j].length; k++) {
						if (
							noam.util.contains(grammar.nonterminals, slimProds[i][1][j][k])
						) {
		...
		noam.re = (function () {
			var tree = (function () {
				...
				/*
        The goal of simplification is to obtain
         a regular expression which
        defines the same language but has a 
        smaller number of elements in
        the tree structure and therefore less
         elements in array or string
        form. 
        
        Simplification is performed by iteratively transforming the
      ...
      */
				...
				function _regex_simplify_7(tree, fsmCache) {
					if (
						tree.tag === tags.KSTAR &&
						tree.expr.tag === tags.SEQ &&
						tree.expr.elements.length > 0
					) {
						var check = true;
				...
				function _regex_simplify_12(tree, fsmCache) {
					if (tree.tag === tags.ALT && tree.choices.length >= 2) {
						for (var i = 0; i < tree.choices.length - 1; i++) {
							...
							for (var j = i + 1; j < tree.choices.length; j++) {
								if (
									tree.choices[j].tag === tags.KSTAR &&
									noam.util.areEquivalent(tree.choices[j].expr, tree.choices[i])
								) {
									found = i;
									break;
								} else if (
									tree.choices[i].tag === tags.KSTAR &&
									noam.util.areEquivalent(tree.choices[i].expr, tree.choices[j])
								) {
									found = j;
									break;
								}
				...
				function _regex_simplify_13(tree, fsmCache) {
					if (tree.tag === tags.SEQ && tree.elements.length >= 2) {
						...
						for (var i = 0; i < tree.elements.length - 1; i++) {
							if (
								tree.elements[i].tag === tags.KSTAR &&
								tree.elements[i + 1].tag === tags.KSTAR &&
								noam.util.areEquivalent(tree.elements[i], tree.elements[i + 1])
							) {
								found = i;
								break;
							}
				...
				function _regex_simplify_14(tree, fsmCache) {
					if (
						tree.tag === tags.KSTAR &&
						tree.expr.tag === tags.ALT &&
						tree.expr.choices.length >= 2
					) {
						for (var i = 0; i < tree.expr.choices.length; i++) {
							for (var j = 0; j < tree.expr.choices.length; j++) {
								if (
									i !== j &&
									tree.expr.choices[j].tag === tags.SEQ &&
									tree.expr.choices[j].elements.length >= 2
								) {
									var found = true;

									for (
										var k = 0;
										k < tree.expr.choices[j].elements.length;
										k++
									) {
										if (
											!noam.util.areEquivalent(
												tree.expr.choices[i],
												tree.expr.choices[j].elements[k]
											)
										) {
											found = false;
											break;
				...
				function _regex_simplify_16(tree, fsmCache) {
					if (tree.tag === tags.ALT && tree.choices.length >= 2) {
						for (var i = 0; i < tree.choices.length - 1; i++) {
							if (
								tree.choices[i].tag === tags.SEQ &&
								tree.choices[i].elements.length >= 2
							) {
								for (var j = i + 1; j < tree.choices.length; j++) {
									if (
										tree.choices[j].tag === tags.SEQ &&
										tree.choices[j].elements.length >= 2
									) {
										if (
											noam.util.areEquivalent(
												tree.choices[j].elements[0],
												tree.choices[i].elements[0]
											)
										) {
				...
				function _regex_simplify_20(tree, fsmCache) {
					if (
						...
						fsms.push(
							getFromCacheOrCreateFsm(makeKStar(tree.expr.choices[0]), fsmCache)
						);
						...
						for (var i = 0; i < tree.expr.choices.length - 1; i++) {
							for (var j = i + 1; j < tree.expr.choices.length; j++) {
								if (fsms.length <= j) {
									fsms.push(
										getFromCacheOrCreateFsm(
											makeKStar(tree.expr.choices[j]),
											fsmCache
										)
									);
								}
				...
				function _regex_simplify_21(tree, fsmCache) {
					if (tree.tag === tags.SEQ && tree.elements.length >= 2) {
						...
						for (var i = 0; i < tree.elements.length - 1; i++) {
							fsms.push(
								getFromCacheOrCreateFsm(tree.elements[i + 1], fsmCache)
							);
				...
				function _regex_simplify_23(tree, fsmCache) {
					if (tree.tag === tags.SEQ && tree.elements.length > 1) {
						for (var i = 0; i < tree.elements.length - 1; i++) {
							if (
								tree.elements[i].tag === tags.KSTAR &&
								tree.elements[i + 1].tag === tags.ALT &&
								tree.elements[i + 1].choices.length === 2
							) {
								var index_eps = noam.util.index(
									tree.elements[i + 1].choices,
									makeEps()
								);
								...
								if (index_eps >= 0) {
									var internal =
										index_eps === 0
											? tree.elements[i + 1].choices[1]
											: tree.elements[i + 1].choices[0];

									if (
										internal.tag === tags.SEQ &&
										internal.elements.length === 2
									) {
										if (
											internal.elements[1].tag === tags.KSTAR &&
											internal.elements[1].expr.tag === tags.ALT &&
											internal.elements[1].expr.choices.length === 2 &&
											noam.util.contains(
												internal.elements[1].expr.choices,
												tree.elements[i].expr
											)
										) {
											if (
												noam.util.contains(
							...
							if (
								...
								if (index_eps >= 0) {
									...
									if (
										...
										if (
											internal.elements[0].tag === tags.KSTAR &&
											internal.elements[0].expr.tag === tags.ALT &&
											internal.elements[0].expr.choices.length === 2 &&
											noam.util.contains(
												internal.elements[0].expr.choices,
												tree.elements[i].expr
											)
										) {
											if (
						...
						for (var i = 0; i < tree.elements.length; i++) {
							if (tree.elements[i].tag === tags.KSTAR) {
								if (
									i > 0 &&
									tree.elements[i - 1].tag === tags.ALT &&
									tree.elements[i - 1].choices.length > 1
								) {
									var index_eps = noam.util.index(
										tree.elements[i - 1].choices,
										makeEps()
									...
									if (index_eps >= 0) {
										var eps = tree.elements[i - 1].choices.splice(
											index_eps,
											1
										)[0];

										var fsm_kstar = getFromCacheOrCreateFsm(
											tree.elements[i],
											fsmCache
										);
										var fsm_other = getFromCacheOrCreateFsm(
											tree.elements[i - 1],
											fsmCache
										);
				...
				function _regex_simplify_25(tree, fsmCache) {
					if (tree.tag === tags.SEQ && tree.elements.length >= 1) {
						for (var i = 0; i < tree.elements.length; i++) {
							if (
								(tree.elements[i].tag === tags.SEQ &&
									tree.elements[i].elements.length === 0) ||
								(tree.elements[i].tag === tags.ALT &&
									tree.elements[i].choices.length === 0)
							) {
								tree.elements = [];
								return true;
							}
				...
				_regex_simplification_patterns.push({
					pattern: "(a()) => ()",
					type: "structure",
					function: _regex_simplify_25,
				});
				_regex_simplification_patterns.push({
					pattern: "()* => ()",
					type: "structure",
					function: _regex_simplify_26,
				});
				_regex_simplification_patterns.push({
					pattern: "(a) => a",
					type: "structure",
					function: _regex_simplify_1,
				});
				_regex_simplification_patterns.push({
					pattern: "(a) => a",
					type: "structure",
					function: _regex_simplify_2,
				});
				_regex_simplification_patterns.push({
					pattern: "$* => $",
					type: "structure",
					function: _regex_simplify_3,
				});
				_regex_simplification_patterns.push({
					pattern: "(a*)* => a*",
					type: "structure",
					function: _regex_simplify_4,
				});
				_regex_simplification_patterns.push({
					pattern: "(a+b*)* => (a+b)*",
					type: "structure",
					function: _regex_simplify_5,
				});
				_regex_simplification_patterns.push({
					pattern: "$+a* => a*",
					type: "structure",
					function: _regex_simplify_6,
				});
				_regex_simplification_patterns.push({
					pattern: "(a*b*)* => (a*+b*)*",
					type: "structure",
					function: _regex_simplify_7,
				});
				_regex_simplification_patterns.push({
					pattern: "$a => a",
					type: "structure",
					function: _regex_simplify_8,
				});
				_regex_simplification_patterns.push({
					pattern: "a+a => a",
					type: "structure",
					function: _regex_simplify_11,
				});
				_regex_simplification_patterns.push({
					pattern: "a+a* => a*",
					type: "structure",
					function: _regex_simplify_12,
				});
				_regex_simplification_patterns.push({
					pattern: "a*a* => a*",
					type: "structure",
					function: _regex_simplify_13,
				});
				_regex_simplification_patterns.push({
					pattern: "(aa+a)* => (a)*",
					type: "structure",
					function: _regex_simplify_14,
				});
				_regex_simplification_patterns.push({
					pattern: "(a + $)* => (a)*",
					type: "structure",
					function: _regex_simplify_15,
				});
				_regex_simplification_patterns.push({
					pattern: "(ab+ac) => a(b+c)",
					type: "structure",
					function: _regex_simplify_16,
				});
				_regex_simplification_patterns.push({
					pattern: "a*aa* => aa*",
					type: "structure",
					function: _regex_simplify_17,
				});
				_regex_simplification_patterns.push({
					pattern: "(ab+cb) => (a+c)b",
					type: "structure",
					function: _regex_simplify_18,
				});
				_regex_simplification_patterns.push({
					pattern: "a*($+b(a+b)*) => (a+b)*",
					type: "structure",
					function: _regex_simplify_23,
				});
				_regex_simplification_patterns.push({
					pattern: "($+(a+b)*a)b* => (a+b)*",
					type: "structure",
					function: _regex_simplify_24,
				});
				_regex_simplification_patterns.push({
					pattern: "$+L => L, if L contains $",
					type: "fsm",
					function: _regex_simplify_22,
				});
				_regex_simplification_patterns.push({
					pattern: "L1+L2 => L2, if L1 is subset of L2",
					type: "fsm",
					function: _regex_simplify_19,
				});
				_regex_simplification_patterns.push({
					pattern: "(L1+L2)* => L2, if L1* is subset of L2*",
					type: "fsm",
					function: _regex_simplify_20,
				});
				_regex_simplification_patterns.push({
					pattern: "L1*L2* => L2, if L1* is subset of L2*",
					type: "fsm",
					function: _regex_simplify_21,
				});
				_regex_simplification_patterns.push({
					pattern: "(L1+$)(L2)* => (L2)* if L1 is subset of L2",
					type: "fsm",
					function: _regex_simplify_27,
				});
				_regex_simplification_patterns.push({
					pattern: "ab(cd) => abcd",
					type: "structure",
					function: _regex_simplify_10,
				});
				_regex_simplification_patterns.push({
					pattern: "(a+(b+c)) => a+b+c",
					type: "structure",
					function: _regex_simplify_9,
				});
				...
				function simplify(tree, config) {
					...
					while (
						appliedPattern !== null &&
						(opts.numIterations === null || iterCount < opts.numIterations)
					) {
						appliedPattern = _simplify_iteration(
							treeClone,
							fsmCache,
							opts.useFsmPatterns
						);
				...
				function _simplify_recursion(tree, patternFunction, fsmCache) {
					...
					for (var i = 0; i < children.length; i++) {
						appliedPattern = _simplify_recursion(
							children[i],
							patternFunction,
							fsmCache
						);
				...
				function _altToAutomaton(regex, automaton, stateCounter) {
					...
					for (var i = 0; i < regex.choices.length; i++) {
						var statePair = _dispatchToAutomaton(
							regex.choices[i],
							automaton,
							stateCounter
						);
				...
				function _seqToAutomaton(regex, automaton, stateCounter) {
					...
					for (var i = 0; i < regex.elements.length; i++) {
						statePair = _dispatchToAutomaton(
							regex.elements[i],
							automaton,
							stateCounter
						);
						if (i === 0) {
							...
							// this is a later element that needs to be connected to the previous elements
			...
			var array = (function () {
				...
				specials.ALT.toString = function () {
					return "+";
				};
				specials.KSTAR.toString = function () {
					return "*";
				};
				specials.LEFT_PAREN.toString = function () {
					return "(";
				};
				specials.RIGHT_PAREN.toString = function () {
					return ")";
				};
				specials.EPS.toString = function () {
					return "$";
				};
				...
				function _replacementStr(obj) {
					...
					if (
						obj === specials.ALT ||
						obj === specials.KSTAR ||
						obj === specials.LEFT_PAREN ||
						obj === specials.RIGHT_PAREN ||
						obj === specials.EPS
					) {
						return obj.toString();
					} else {
				...
				function _parseAtom(input) {
					if (input.peek() === specials.LEFT_PAREN) {
						...
						input.peek() === undefined ||
						input.peek() === specials.ALT ||
				...
				function random(numSymbols, alphabet, cfg) {
					return noam.re.tree.toArray(
						noam.re.tree.random(numSymbols, alphabet, cfg)
					);
				}

				function simplify(arr, numIterations, appliedPatterns) {
					var tree = noam.re.array.toTree(arr);
					var treeSimplified = noam.re.tree.simplify(
						tree,
						numIterations,
						appliedPatterns
					);
			...
			var string = (function () {
				...
				function toArray(str) {
					...
					for (var i = 0; i < str.length; ++i) {
						if (escaped) {
							if (escapable.indexOf(str[i]) === -1) {
								throw new RegexError(
									"Malformed string regex: illegal escape sequence \\" + str[i],
									i
								);
							}
							...
							switch (chr) {
								...
								case "*":
									...
									break;
								case "(":
									chr = specials.LEFT_PAREN;
									break;
								case ")":
									chr = specials.RIGHT_PAREN;
									break;
							}
							arr.push(chr);
						}
					...
					if (escaped) {
						throw new RegexError(
							"Malformed string regex: unfinished escape sequence at end of string",
							str.length - 1
						);
					}
				...
				function random(numSymbols, alphabet, cfg) {
					...
					return noam.re.tree.toString(
						noam.re.tree.random(numSymbols, arr, cfg)
					);
				}
